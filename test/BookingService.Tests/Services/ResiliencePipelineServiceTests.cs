...
/// Tests retry logic, exponential backoff, and timeout behavior for BookingService
/// </summary>
public class ResiliencePipelineServiceTests
{
    ...
    [Fact]
    public async Task DatabasePipeline_TimeoutsAfter30Seconds()
    {
        // Arrange
        var pipeline = _sut.GetDatabasePipeline();
        ...
        var act = async () => await pipeline.ExecuteAsync(async ct =>
        {
            await Task.Delay(TimeSpan.FromSeconds(35), ct); // Longer than timeout
        });
    ...
    [Fact]
    public async Task DatabasePipeline_HandlesMultipleConcurrentCalls()
    {
        // Arrange
        var pipeline = _sut.GetDatabasePipeline();
        var successCount = 0;
        ...
        // Assert
        successCount.Should().Be(10, "all concurrent database operations should succeed");
    }
    ...
    [Fact]
    public async Task DatabasePipeline_ExponentialBackoff_IncreasesDelay()
    {
        // Arrange
        var pipeline = _sut.GetDatabasePipeline();
        var attemptTimes = new List<DateTime>();
        ...
        await pipeline.ExecuteAsync(async ct =>
        {
            ...
            if (attemptTimes.Count < 3)
            {
                throw new TimeoutException("Database retry test");
            }
            await Task.CompletedTask;
        });
        ...
        delay1.Should().BeGreaterThan(0.75, "first retry should have ~1s delay");
        delay2.Should().BeGreaterThan(1.5, "second retry should have ~2s delay");
    }
    ...
    [Fact]
    public async Task EventPublishingPipeline_HandlesOperationCancellation()
    {
        // Arrange
        var pipeline = _sut.GetEventPublishingPipeline();
    ...
    [Fact]
    public async Task DatabasePipeline_HandlesOperationCancellation()
    {
        // Arrange
        var pipeline = _sut.GetDatabasePipeline();
        using var cts = new CancellationTokenSource();
